...
let g:StslineColorGreen  = "#A6DA95"
let g:StslineColorBlue   = "#8AADF4"
let g:StslineColorViolet = "#A09CF7"
let g:StslineColorYellow = "#EED49F"
let g:StslineColorOrange = "#F5A97F"
 
let g:StslineColorLight  = "#C0C0C0"
let g:StslineColorDark   = "#080808"
let g:StslineColorDark1  = "#181818"
let g:StslineColorDark2  = "#1f2132"
let g:StslineColorDark3  = "#6A5FAD"
...
" %F, %c, etc. 								= Variables which contain value like - current file path, current colums, etc.
" %{&readonly?\"\ \":\"\"} 				= If file is readonly ? Then "Lock icon" Else : "Nothing"
" %{get(b:,'coc_git_status',b:GitBranch)}	= If b:coc_git_status exists, then it's value, else value of b:GitBranch
...
" TODO: Contents of statusline are evaluated every screen update. Meaning
" expensive functions like getting a git brance name or date for example, can
" be quite expensive. How to optimize?

" Define active statusline

function! ActivateStatusline()
call GetFileType()
" Draw current vim mode.
setlocal statusline=%#StslinePriColorBG#\ %{StslineMode()}

" Draw git info
setlocal statusline+=%#StslineSecColorBG#%{get(b:,'coc_git_status',b:GitBranch)}%{get(b:,'coc_git_blame','')}

" Draw rounded right border.
setlocal statusline+=%#StslineBackColorFGPriColorBG#

" Display readonly info
setlocal statusline+=%#StslinePriColorFG#\ %{&readonly?\"\ \":\"\"}

" Display file path & Modified info
setlocal statusline+=%F\ %#StslineModColorFG#%{&modified?\"\ \":\"\"}

" Switch to right side & Display current file type.
setlocal statusline+=%=%#StslinePriColorFG#\ %{b:FiletypeIcon}%{&filetype}

" Draw left icon separators.
setlocal statusline+=\ %#StslineSecColorFG#%#StslineSecColorBG#%{&fenc!='utf-8'?\"\ \":''}%{&fenc!='utf-8'?&fenc:''}%{&fenc!='utf-8'?\"\ \":''}

" Draw right icon separator.
setlocal statusline+=%#StslinePriColorFGSecColorBG#%#StslinePriColorBG#

" Display current cursor data.
setlocal statusline+=\ %p\%%\ %#StslinePriColorBGBold#%l%#StslinePriColorBG#/%L\ :%c\ 
endfunction
...
if !exists("b:GitBranch") || b:GitBranch == ''
execute 'highlight StslineBackColorFGPriColorBG guifg=' . g:StslinePriColor ' guibg=' . g:StslineBackColor
endif
...
try
    ...
    if !v:shell_error
        ...
        execute 'highlight StslineBackColorFGPriColorBG guifg=' . g:StslineSecColor ' guibg=' . g:StslineBackColor
    endif
catch
endtry
endfunction
